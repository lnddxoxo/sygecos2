...
// (This section remains as is for internal logic, but the dropdown will be filtered)
$contacts = [];
try {
    // Enseignants
    $stmt = $pdo->query("
        SELECT 'enseignant' as type, id_ens as id_ref, fk_id_util as id_util, CONCAT(nom_ens, ' ', prenom_ens) as nom, email, 'Enseignant' as role
        FROM enseignant
        ORDER BY nom_ens, prenom_ens
    ");
    $enseignants = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Personnel administratif (exclure le chargé de communication lui-même)
    $stmt = $pdo->prepare("
        SELECT 'personnel' as type, id_pers as id_ref, fk_id_util as id_util, CONCAT(nom_pers, ' ', prenoms_pers) as nom, email_pers as email, poste as role
    ...
    // Étudiants
    $stmt = $pdo->query("
        SELECT 'etudiant' as type, num_etu as id_ref, fk_id_util as id_util, CONCAT(nom_etu, ' ', prenoms_etu) as nom, email_etu as email,
    ...
    // Entreprises partenaires (email_contact est l'identifiant pour l'envoi)
    $stmt = $pdo->query("
        SELECT 'entreprise' as type, id_entr as id_ref, NULL as id_util, lib_entr as nom, email_contact as email, 'Entreprise Partenaire' as role
        FROM entreprise
    ...
    $contacts = array_merge($enseignants, $personnel, $etudiants, $entreprises);
...
// Récupérer les ID_GU pour le rôle "Chargé de communication"
$chargeCommunicationGUId = null;
try {
    $stmt = $pdo->query("SELECT id_GU, lib_GU FROM groupe_utilisateur WHERE lib_GU = 'Chargé de communication'");
    $gu_ids = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
    if (isset($gu_ids['Chargé de communication'])) {
        $chargeCommunicationGUId = $gu_ids['Chargé de communication'];
    }
} catch (PDOException $e) {
    error_log("Erreur récupération ID_GU: " . $e->getMessage());
}

// Récupérer les ID_UTIL des destinataires pour la liste déroulante (uniquement Chargé de communication)
$specificRecipientsForDropdown = [];
try {
    if ($chargeCommunicationGUId) {
        $stmtUser = $pdo->prepare("
            SELECT u.id_util, u.login_util,
                   COALESCE(e.nom_ens, pa.nom_pers) as nom,
                   COALESCE(e.prenom_ens, pa.prenoms_pers) as prenom,
                   COALESCE(e.email, pa.email_pers) as email
            FROM utilisateur u
            ...
            WHERE p.fk_id_GU = ? LIMIT 1
        "); // Added LIMIT 1 to get only one 'Chargé de communication'
        $stmtUser->execute([$chargeCommunicationGUId]);
        $user = $stmtUser->fetch(PDO::FETCH_ASSOC);

        if ($user) {
            $displayName = trim($user['prenom'] . ' ' . $user['nom']);
            if (empty($displayName)) {
                $displayName = $user['login_util'];
            }
            $specificRecipientsForDropdown[] = [
    ...
    error_log("Erreur récupération utilisateurs spécifiques pour dropdown: " . $e->getMessage());
}
    ...
    try {
        ...
        switch ($action) {
            case 'charger_messages':
                ...
                if ($dossier !== 'rapports') { // Get all relevant messages for total counts
                    $stmtAll = $pdo->prepare("
                    ...
                    // If we filtered by 'rapports', need to get all reports for total count for that filter
                    $stmtAll = $pdo->prepare("
                        SELECT m.id_message, m.is_read, m.message_type, m.subject, m.receiver_id_util, m.sender_id_util
                        FROM messages m
                        WHERE m.receiver_id_util = :loggedInUserId AND m.message_type = 'rapport'
                    ");
                    $stmtAll->bindParam(':loggedInUserId', $loggedInUserId, PDO::PARAM_INT);
                    $stmtAll->execute();
                    $allMessagesForCounts = $stmtAll->fetchAll(PDO::FETCH_ASSOC);
                ...
                $totalMessages = count($allMessagesForCounts);
                ...
                // Add logic here for 'important', 'draft', 'trash' if you have columns in the DB

                $pagesTotal = ceil($totalMessages / $limite);
                ...
                echo json_encode([
                    ...
                    'counts' => [
                        'reception' => count(array_filter($allMessagesForCounts, function($m) use ($loggedInUserId) { return $m['receiver_id_util'] == $loggedInUserId; })),
                        'non_lus' => $nonLusCount,
            ...
            case 'envoyer_message':
                $destinataireIds = $_POST['destinataire_ids'] ?? []; // Array of id_util
                $sujet = trim($_POST['sujet'] ?? '');
                $contenu = trim($_POST['contenu'] ?? '');
                $messageType = $_POST['message_type'] ?? 'general'; // Default to general as type field is removed
                $reportDescription = $_POST['report_description'] ?? null;
                $academicYearId = intval($_POST['academic_year_id'] ?? 0);

                if (empty($destinataireIds) || empty($sujet) || empty($contenu)) {
                    throw new Exception("All fields (recipients, subject, content) are mandatory.");
                }
                // If it's a report type, ensure report-specific fields are provided
                if ($messageType === 'rapport') {
                    if (empty($reportDescription) || $academicYearId <= 0) {
                        throw new Exception("Report description and academic year are mandatory for a report.");
                    }
                }
                ...
                foreach ($destinataireIds as $receiverId) {
                    ...
                    if ($messageType === 'rapport') {
                        ...
                        $stmtReport->execute([
                            NULL, // Placeholder for fk_num_etu - adjust if reports are tied to a specific student from this form
                            $dummyFileName,
                            $dummyFilePath,
                            $academicYearId,
                            $reportDescription
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Specific styles for the new select dropdown */
        #recipientSelect {
        ...
        .recipients-input-container, #recipientsSearchInput, .recipient-tag, .contact-suggestions, .contact-suggestion, .contact-avatar-sm, .contact-info {
            display: none !important; /* Hide all elements related to the old tag input */
        }
        ...
        /* Hide the cc-cci toggle for now as no functionality is added */
        .cc-cci-toggle {
            display: none;
        }
        ...
        @media (max-width: 768px) {
            ...
            .report-print-preview { grid-template-columns: 1fr; } /* Stack options and preview */
...
<body>
            ...
            <div class="mail-composer-view" id="mailComposerView">
                <div class="compose-header" id="composeHeader">
                    ...
                    <div class="compose-header-actions">
                        ...
                        <button class="compose-close btn-icon" id="closeComposerBtn" title="Fermer">&times;</button>
                    </div>
                </div>
                <form class="compose-form" id="mainComposeForm">
                    <div class="composer-fields">
                        <div class="composer-field">
                            ...
                            <select id="recipientSelect" class="form-control" required>
                                ...
                                <?php foreach ($specificRecipientsForDropdown as $recipient): ?>
                                    <option value="<?php echo htmlspecialchars($recipient['id_util']); ?>" data-role="<?php echo htmlspecialchars($recipient['role']); ?>">
                                        <?php echo htmlspecialchars($recipient['name']) . ' (' . htmlspecialchars($recipient['role']) . ')'; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <span class="cc-cci-toggle" style="margin-left: var(--space-2); color: var(--gray-500); font-size: var(--text-sm); cursor: pointer;">Cc Cci</span>
                        </div>
                        <div class="composer-field">
                            <label>Objet</label>
                    ...
                    <div class="compose-editor">
                        <textarea id="composeMessageContent" required></textarea>
                    </div>
                    <div id="reportSpecificFields" class="hidden">
                        <div class="composer-field">
                            <label>Description:</label>
                            <textarea id="reportDescription" name="report_description" rows="3" placeholder="Brève description du rapport"></textarea>
                        </div>
                        <div class="composer-field">
                            <label>Année Acad.:</label>
                            <select id="academicYearSelect" name="academic_year_id">
                        ...
                        <div style="text-align: right; padding: var(--space-3) var(--space-4);">
                            <button type="button" class="btn btn-success" id="depositReportBtn">
                                <i class="fas fa-file-export"></i> Déposer Rapport
                            </button>
                        </div>
                    </div>
    ...
    <div class="message-modal" id="messageAlertModal">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <span class="close" onclick="closeModal('messageAlertModal')">&times;</span>
            </div>
    ...
    <script>
        ...
        const specificRecipientsForDropdown = <?php echo json_encode($specificRecipientsForDropdown); ?>;
        ...
        function closeModal(modalId) {
            ...
            if (modalId === 'reportPreviewModal') {
                ...
                sendMailBtn.textContent = 'Envoyer';
            ...
            if (modalId === 'mailComposerView') {
                ...
                recipientSelect.value = ''; // Ensure dropdown is reset
                reportSpecificFields.classList.add('hidden');
                reportDescription.removeAttribute('required');
        ...
        function showMailComposerView() {
            ...
            mainComposeForm.reset();
            // Automatically select the 'Chargé de communication' if it's the only option
            if (specificRecipientsForDropdown.length === 1) {
                recipientSelect.value = specificRecipientsForDropdown[0].id_util;
                // Manually trigger the change event to handle report-specific fields
                const event = new Event('change');
                recipientSelect.dispatchEvent(event);
            } else {
                recipientSelect.value = ''; // Clear selection if multiple/none for some reason
            }
            ...
            // The recipientSelect.dispatchEvent(event) above should handle this,
            // but for safety, ensure report fields are managed correctly on compose view open
            const selectedOption = recipientSelect.options[recipientSelect.selectedIndex];
            const selectedRole = selectedOption ? selectedOption.dataset.role : null;
            if (selectedRole === 'Chargé de communication') {
                reportSpecificFields.classList.remove('hidden');
                reportDescription.setAttribute('required', 'required');
                academicYearSelect.setAttribute('required', 'required');
                sendMailBtn.setAttribute('disabled', 'disabled'); // Disable direct send
            } else {
                reportSpecificFields.classList.add('hidden');
                reportDescription.removeAttribute('required');
                academicYearSelect.removeAttribute('required');
        ...
        function displayFullMessage(message) {
            ...
            let contentHtml = `<p>${message.body.replace(/\n/g, '<br>')}</p>`;

            if (message.message_type === 'rapport' && message.report_description) {
                contentHtml += `
                    <div class="report-details-box">
                        <strong>Détails du Rapport Associé:</strong><br>
                        Année Académique: ${htmlspecialchars(message.report_annee_academique_libelle) || 'N/A'}<br>
                        Description: ${htmlspecialchars(message.report_description).replace(/\n/g, '<br>')}<br>
                        <em>(Ce rapport a été déposé conceptuellement via la messagerie.)</em>
                    </div>
                `;
        ...
        mainComposeForm.addEventListener('submit', async function(e) {
            ...
            const selectedOption = recipientSelect.options[recipientSelect.selectedIndex];
            const selectedRecipientRole = selectedOption.dataset.role;

            let messageType = 'general'; // Default message type

            // If the recipient is 'Chargé de communication', we assume it's a report deposit flow
            if (selectedRecipientRole === 'Chargé de communication') {
                messageType = 'rapport';

                if (reportDescription.value.trim() === '' || academicYearSelect.value === '') {
                    showAlert("Pour un rapport, veuillez remplir la description et l'année académique, puis cliquez sur 'Déposer Rapport'.", 'warning');
                    return;
                }
                // If deposit button was not clicked, prevent direct send through general submit
                // The actual send will happen via confirmSendReportBtn
                showAlert("Pour déposer un rapport, utilisez le bouton 'Déposer Rapport' pour la prévisualisation.", 'warning');
                return;
            }

            await sendFinalMessage(messageType);
        });

        // Send message function (called by submit or from report preview modal)
        async function sendFinalMessage(messageType = 'general') {
            showLoading(true);

            const recipientId = recipientSelect.value;
            const recipientName = recipientSelect.options[recipientSelect.selectedIndex].text; // Get display name

            try {
                const dataToSend = {
                    action: 'envoyer_message',
                    destinataire_ids: [recipientId],
                    sujet: composeSubjectInput.value,
                    contenu: composeMessageContent.value,
                    message_type: messageType,
                };

                // Only add report specific data if it's a report type
                if (messageType === 'rapport') {
                    dataToSend.report_description = reportDescription.value;
                    dataToSend.academic_year_id = academicYearSelect.value;
                }

                const result = await makeAjaxRequest(dataToSend);

                if (result.success) {
                    showAlert(`Message envoyé à ${recipientName}.`, 'success');
        ...
        depositReportBtn.addEventListener('click', function() {
            ...
            const selectedRecipientId = recipientSelect.value;
            const selectedRecipientName = recipientSelect.options[recipientSelect.selectedIndex]?.text;


            if (!selectedRecipientId) {
                showAlert('Veuillez sélectionner un destinataire pour le rapport.', 'warning');
                return;
            }
            if (!subject) {
            ...
            currentReportPreviewData = {
                ...
                messageType: 'rapport' // Explicitly set message type for sending
            };
            ...
            previewRecipientsInput.value = selectedRecipientName;
            ...
            reportPreviewContent.innerHTML = `
                ...
                <p class="preview-item"><strong>Destinataire:</strong> ${htmlspecialchars(selectedRecipientName)}</p>
        ...
        confirmSendReportBtn.addEventListener('click', async function() {
            if (currentReportPreviewData) {
                ...
                try {
                    const dataToSend = {
                        action: 'envoyer_message',
                        destinataire_ids: [currentReportPreviewData.recipientId],
                        sujet: currentReportPreviewData.subject,
                        contenu: currentReportPreviewData.body || `Dépôt de Rapport - Année Académique: ${currentReportPreviewData.academicYearText}\nDescription: ${currentReportPreviewData.reportDescription}`,
        ...
        function populateAcademicYearSelects() {
            ...
            previewAcademicYearSelect.innerHTML = '';
        ...
        // Populate the new recipient select dropdown
        function populateRecipientSelect() {
            recipientSelect.innerHTML = '<option value="">Sélectionner un destinataire</option>';
            // Only add 'Chargé de communication' if available in the specificRecipientsForDropdown
            specificRecipientsForDropdown.forEach(recipient => {
                const option = document.createElement('option');
                option.value = recipient.id_util;
                option.textContent = `${htmlspecialchars(recipient.name)} (${htmlspecialchars(recipient.role)})`;
                option.dataset.role = recipient.role; // Store role in data-attribute
                recipientSelect.appendChild(option);
            });

            // If there's only one recipient (which should be "Chargé de communication"), pre-select it
            if (specificRecipientsForDropdown.length === 1) {
                recipientSelect.value = specificRecipientsForDropdown[0].id_util;
                // Manually trigger the change event to ensure dependent fields (report-specific) are updated
                const event = new Event('change');
                recipientSelect.dispatchEvent(event);
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            recipientSelect.addEventListener('change', function() {
                ...
                if (selectedRole === 'Chargé de communication') {
                    ...
                    reportDescription.removeAttribute('required');
                    academicYearSelect.removeAttribute('required');
                    sendMailBtn.removeAttribute('disabled'); // Enable direct send for other recipients
                }
            });
            ...
            replyBtn.addEventListener('click', function() {
                ...
                if (senderUserId) {
                    // Find the sender in the specificRecipientsForDropdown to set the value and trigger change
                    const foundRecipient = specificRecipientsForDropdown.find(rec => rec.id_util == senderUserId);
                    if (foundRecipient) {
                        recipientSelect.value = senderUserId;
                        const event = new Event('change');
                        recipientSelect.dispatchEvent(event); // Trigger change to handle report fields visibility
                    } else {
                        showAlert("Impossible de trouver l'expéditeur original dans la liste des destinataires internes.", "warning");
                        recipientSelect.value = ''; // Clear selection if not found
                        reportSpecificFields.classList.add('hidden'); // Ensure hidden
                        sendMailBtn.removeAttribute('disabled'); // Ensure send is enabled
                    }
                } else {
                    showAlert("L'expéditeur original n'a pas d'ID utilisateur associé, impossible de répondre via ce système.", "warning");
                    recipientSelect.value = ''; // Clear selection
                    reportSpecificFields.classList.add('hidden'); // Ensure hidden
                    sendMailBtn.removeAttribute('disabled'); // Ensure send is enabled
                }
            ...
            forwardBtn.addEventListener('click', function() {
                ...
                showMailComposerView();
                recipientSelect.value = ''; // Clear recipient for forwarding
                reportSpecificFields.classList.add('hidden'); // Ensure report fields are hidden when forwarding
                sendMailBtn.removeAttribute('disabled'); // Ensure send button is enabled
