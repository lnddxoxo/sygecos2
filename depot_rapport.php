...
// Simulate a logged-in student (replace with actual session data)
// In a real application, you would get this from your authentication system
if (!isset($_SESSION['user_id']) || !isset($_SESSION['user_role'])) {
    // Redirect to login page or show an error
    // header('Location: login.php');
    // exit();
    $_SESSION['user_id'] = 5; // Example: Assuming user_id 5 is an student
    $_SESSION['user_role'] = 'etudiant';
}

// Simulate fetching student and filiere data from the database
// You'll replace this with actual database queries using PDO or mysqli
$studentData = [];
$filiereData = [];
$activeAcademicYear = '';
$activeAcademicYearId = 0; // Store the ID as well
$companies = [];
...
try {
    ...
    $stmt = $pdo->prepare("SELECT id_Ac, CONCAT(YEAR(date_deb), '-', YEAR(date_fin)) AS annee_libelle FROM année_academique WHERE statut = 'active' AND est_courante = 1 LIMIT 1");
    $stmt->execute();
    $academicYearResult = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($academicYearResult) {
        $activeAcademicYear = $academicYearResult['annee_libelle'];
        $activeAcademicYearId = $academicYearResult['id_Ac'];
    }

    // Fetch student data based on logged-in user ID
    $studentId = $_SESSION['user_id'];
    $stmt = $pdo->prepare("SELECT e.num_etu, e.nom_etu, e.prenoms_etu, f.lib_filiere
                            FROM etudiant e
                            JOIN filiere f ON e.fk_id_filiere = f.id_filiere
                            WHERE e.fk_id_util = :student_id");
    $stmt->bindParam(':student_id', $studentId);
    $stmt->execute();
    $studentData = $stmt->fetch(PDO::FETCH_ASSOC);

    // Fetch companies for the dropdown
    $stmt = $pdo->query("SELECT id_entr, lib_entr FROM entreprise ORDER BY lib_entr ASC");
    $companies = $stmt->fetchAll(PDO::FETCH_ASSOC);

} catch (PDOException $e) {
    // In a real application, log the error and show a user-friendly message
...
// Initialize session variables for report if not set
if (!isset($_SESSION['current_report_id'])) {
    $_SESSION['current_report_id'] = null;
    $_SESSION['report_metadata'] = [];
    $_SESSION['report_content'] = ''; // Store the HTML content
    $_SESSION['show_editor'] = false;
}

// Try to load existing report if student has one for the current academic year
if ($studentData && $activeAcademicYearId) {
    $stmt = $pdo->prepare("SELECT id_rapport, theme_rapport, universite, nom_encadrant, entreprise, periode_stage, contenu_rapport, statut
                            FROM rapports
                            WHERE fk_num_etu = :num_etu AND fk_id_Ac = :id_ac
                            LIMIT 1");
    $stmt->bindParam(':num_etu', $studentData['num_etu']);
    $stmt->bindParam(':id_ac', $activeAcademicYearId);
    $stmt->execute();
    $existingReport = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($existingReport) {
        $_SESSION['current_report_id'] = $existingReport['id_rapport'];
        $_SESSION['report_metadata'] = [
            'reportTheme' => $existingReport['theme_rapport'],
            'university' => $existingReport['universite'],
            'faculty' => $studentData['lib_filiere'],
            'studentName' => ($studentData['prenoms_etu'] ?? '') . ' ' . ($studentData['nom_etu'] ?? ''),
            'studentId' => $studentData['num_etu'],
            'supervisorName' => $existingReport['nom_encadrant'],
            'companyName' => $existingReport['entreprise'],
            'period' => $existingReport['periode_stage'],
            'academicYear' => $activeAcademicYear,
            'statut' => $existingReport['statut']
        ];
        $_SESSION['report_content'] = $existingReport['contenu_rapport'];
        $_SESSION['show_editor'] = true; // Show editor directly if report exists
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['submit_metadata'])) {
        $reportTheme = $_POST['reportTheme'] ?? '';
        $university = $_POST['university'] ?? '';
        $faculty = $studentData['lib_filiere'] ?? ''; // Pre-filled
        $studentName = ($studentData['prenoms_etu'] ?? '') . ' ' . ($studentData['nom_etu'] ?? ''); // Pre-filled
        $studentNumEtu = $studentData['num_etu'] ?? ''; // Pre-filled
        $supervisorName = $_POST['supervisorName'] ?? '';
        $companyName = $_POST['companyName'] ?? '';
        $newCompanyName = $_POST['newCompanyName'] ?? '';
        $period = $_POST['period'] ?? '';
        $academicYear = $_POST['academicYear'] ?? ''; // Pre-filled

        // Basic server-side validation
        if (empty($reportTheme)) { $errors[] = "Le thème du stage est requis."; }
        if (empty($university)) { $errors[] = "L'université/école est requise."; }
        if (empty($supervisorName)) { $errors[] = "Le nom de l'encadrant est requis."; }
        if (empty($companyName) && empty($newCompanyName)) { $errors[] = "Le nom de l'entreprise est requis."; }
        if (empty($period)) { $errors[] = "La période de stage est requise."; }
        else {
            if (!preg_match('/^\d{2}\/\d{2}\/\d{2} - \d{2}\/\d{2}\/\d{2}$/', $period)) {
                $errors[] = "Le format de la période de stage doit être JJ/MM/AA - JJ/MM/AA.";
            } else {
                list($startDateStr, $endDateStr) = explode(' - ', $period);
                try {
                    $startDate = DateTime::createFromFormat('d/m/y', $startDateStr);
                    $endDate = DateTime::createFromFormat('d/m/y', $endDateStr);

                    if (!$startDate || !$endDate) {
                        $errors[] = "Dates de la période de stage invalides.";
                    } elseif ($startDate >= $endDate) {
                        $errors[] = "La date de début doit être antérieure à la date de fin.";
                    } else {
                        $interval = $startDate->diff($endDate);
                        if ($interval->m < 1 && $interval->y == 0) {
                            $errors[] = "La période de stage doit durer au moins un mois.";
                        }
                    }
                } catch (Exception $e) {
                    $errors[] = "Erreur de parsing des dates de stage.";
                }
            }
        }

        if (empty($errors)) {
            // If 'newCompanyName' is provided, insert it into the database
            if (!empty($newCompanyName) && $companyName === 'other') {
                try {
                    $stmt = $pdo->prepare("INSERT INTO entreprise (lib_entr) VALUES (:new_company_name)");
                    $stmt->bindParam(':new_company_name', $newCompanyName);
                    $stmt->execute();
                    // $companyId = $pdo->lastInsertId(); // Not strictly needed for metadata, using name
                    $companyName = $newCompanyName; // Use the newly added company name
                } catch (PDOException $e) {
                    $errors[] = "Erreur lors de l'ajout de la nouvelle entreprise: " . $e->getMessage();
                }
            }

            if (empty($errors)) { // Check errors again after company insert attempt
                $_SESSION['report_metadata'] = [
                    'reportTheme' => $reportTheme,
                    'university' => $university,
                    'faculty' => $faculty,
                    'studentName' => $studentName,
                    'studentId' => $studentNumEtu,
                    'supervisorName' => $supervisorName,
                    'companyName' => $companyName,
                    'period' => $period,
                    'academicYear' => $activeAcademicYear,
                    'statut' => 'brouillon' // Initial status
                ];
                $_SESSION['show_editor'] = true;
                // If this is a new report creation after submission of metadata,
                // ensure current_report_id is null so save_report logic inserts.
                if (!isset($existingReport) || !$existingReport) {
                    $_SESSION['current_report_id'] = null;
                    $_SESSION['report_content'] = ''; // Start with empty content for new report
                }
            }
        }
    }
    // Handle AJAX actions for saving and deleting report content
    elseif (isset($_POST['action'])) {
        ...
        try {
            if ($_POST['action'] === 'save_report') {
                ...
                if (empty($metadata['reportTheme']) || empty($metadata['university']) || empty($metadata['supervisorName']) ||
                    empty($metadata['companyName']) || empty($metadata['period']) || empty($studentData['num_etu']) ||
                    empty($activeAcademicYearId)) {
                    throw new Exception("Données du rapport incomplètes pour l'enregistrement.");
                }

                if ($reportId) {
                    // Update existing report
                    $stmt = $pdo->prepare("UPDATE rapports SET
                                            theme_rapport = :theme_rapport,
                                            universite = :universite,
                                            nom_encadrant = :nom_encadrant,
                                            entreprise = :entreprise,
                                            periode_stage = :periode_stage,
                                            contenu_rapport = :contenu_rapport,
                                            date_modification = NOW()
                                            WHERE id_rapport = :id_rapport AND fk_num_etu = :fk_num_etu");
                    $stmt->bindParam(':id_rapport', $reportId);
                } else {
                    // Insert new report
                    $stmt = $pdo->prepare("INSERT INTO rapports (fk_num_etu, fk_id_Ac, theme_rapport, universite, nom_encadrant, entreprise, periode_stage, contenu_rapport)
                                            VALUES (:fk_num_etu, :fk_id_Ac, :theme_rapport, :universite, :nom_encadrant, :entreprise, :periode_stage, :contenu_rapport)");
                }

                $stmt->bindParam(':theme_rapport', $metadata['reportTheme']);
                $stmt->bindParam(':universite', $metadata['university']);
                $stmt->bindParam(':nom_encadrant', $metadata['supervisorName']);
                $stmt->bindParam(':entreprise', $metadata['companyName']);
                $stmt->bindParam(':periode_stage', $metadata['period']);
                $stmt->bindParam(':contenu_rapport', $content);
                $stmt->bindParam(':fk_num_etu', $studentData['num_etu']);
                if (!$reportId) { // Only bind fk_id_Ac for insert
                    $stmt->bindParam(':fk_id_Ac', $activeAcademicYearId);
                }
                $stmt->execute();
                ...
                $_SESSION['current_report_id'] = $reportId; // Update session with new report ID

                echo json_encode(['success' => true, 'message' => 'Rapport enregistré avec succès!', 'report_id' => $reportId]);
            }
            elseif ($_POST['action'] === 'delete_report') {
                $reportId = $_POST['report_id'] ?? null;

                if (!$reportId) {
                    throw new Exception("ID du rapport manquant pour la suppression.");
                }

                $stmt = $pdo->prepare("DELETE FROM rapports WHERE id_rapport = :id_rapport AND fk_num_etu = :fk_num_etu");
                $stmt->bindParam(':id_rapport', $reportId);
                $stmt->bindParam(':fk_num_etu', $studentData['num_etu']); // Ensure only the owner can delete
                $stmt->execute();

                if ($stmt->rowCount() > 0) {
                    $_SESSION['current_report_id'] = null; // Clear session report ID
                    $_SESSION['report_metadata'] = []; // Clear metadata
                    $_SESSION['report_content'] = ''; // Clear content
            ...
            // If you want to load report content via AJAX after initial page load (e.g. after metadata submission without full page reload)
            elseif ($_POST['action'] === 'load_report_content') {
                if (isset($_SESSION['report_content'])) {
                     echo json_encode(['success' => true, 'content' => $_SESSION['report_content']]);
                } else {
                    echo json_encode(['success' => false, 'message' => 'Contenu du rapport non trouvé.']);
                }
            }
            else {
...
$metadata = $_SESSION['report_metadata'] ?? [];
$reportContent = $_SESSION['report_content'] ?? '<p><br></p>'; // Default content for new reports or if empty
...
<head>
    ...
    <title>SYGECOS - Dépôt de Rapport</title>
...
<body>
        ...
        <main class="main-content" id="mainContent">
            ...
            <div class="page-content">
                <div class="page-header">
                    <div>
                        <h1 class="page-title-main">Dépôt de Rapport de Stage</h1>
                        <p class="page-subtitle">Rédigez et formatez votre rapport professionnel</p>
                    </div>
                </div>
                ...
                <div class="metadata-form" id="metadataForm">
                    <h2>Informations sur le rapport</h2>
                    ...
                    <form id="reportMetadataForm" method="POST">
                        <div class="form-grid">
                            <div class="form-group">
                                <label for="academicYear">Année Académique</label>
                                <input type="text" id="academicYear" name="academicYear" value="<?php echo htmlspecialchars($activeAcademicYear); ?>" readonly>
                            </div>
                            
                            <div class="form-group">
                                <label for="reportTheme">Thème du stage*</label>
                                <input type="text" id="reportTheme" name="reportTheme" value="<?php echo htmlspecialchars($metadata['reportTheme'] ?? ''); ?>" required>
                            </div>
                            
                            <div class="form-group">
                                <label for="university">Université/École*</label>
                                <input type="text" id="university" name="university" value="<?php echo htmlspecialchars($metadata['university'] ?? ''); ?>" required>
                            </div>
                            
                            <div class="form-group">
                                <label for="faculty">Filière/Spécialité</label>
                                <input type="text" id="faculty" name="faculty" value="<?php echo htmlspecialchars($studentData['lib_filiere'] ?? ''); ?>" readonly>
                            </div>
                            
                            <div class="form-group">
                                <label for="studentName">Nom complet de l'étudiant</label>
                                <input type="text" id="studentName" name="studentName" value="<?php echo htmlspecialchars(($studentData['prenoms_etu'] ?? '') . ' ' . ($studentData['nom_etu'] ?? '')); ?>" readonly>
                            </div>
                            
                            <div class="form-group">
                                <label for="studentId">Numéro d'étudiant</label>
                                <input type="text" id="studentId" name="studentId" value="<?php echo htmlspecialchars($studentData['num_etu'] ?? ''); ?>" readonly>
                            </div>
                            
                            <div class="form-group">
                                <label for="supervisorName">Nom de l'encadrant*</label>
                                <input type="text" id="supervisorName" name="supervisorName" value="<?php echo htmlspecialchars($metadata['supervisorName'] ?? ''); ?>" required>
                            </div>
                            
                            <div class="form-group">
                                <label for="companyName">Nom de l'entreprise*</label>
                                <select id="companyName" name="companyName" onchange="toggleNewCompanyInput()" required>
                                    <option value="">-- Sélectionner une entreprise --</option>
                                    <?php foreach ($companies as $company): ?>
                                        <option value="<?php echo htmlspecialchars($company['lib_entr']); ?>" <?php echo (isset($metadata['companyName']) && $metadata['companyName'] == $company['lib_entr']) ? 'selected' : ''; ?>>
                                            <?php echo htmlspecialchars($company['lib_entr']); ?>
                                        </option>
                                    <?php endforeach; ?>
                            ...
                            <div class="form-group" id="newCompanyGroup" style="display: none;">
                                <label for="newCompanyName">Nouvelle entreprise*</label>
                                <input type="text" id="newCompanyName" name="newCompanyName" value="<?php echo (isset($_POST['newCompanyName']) && $_POST['companyName'] == 'other') ? htmlspecialchars($_POST['newCompanyName']) : ''; ?>" placeholder="Entrez le nom de la nouvelle entreprise">
                            </div>

                            <div class="form-group">
                                <label for="period">Période de stage (JJ/MM/AA - JJ/MM/AA)*</label>
                                <input type="text" id="period" name="period" placeholder="Ex: 01/06/23 - 31/08/23" pattern="\d{2}/\d{2}/\d{2} - \d{2}/\d{2}/\d{2}" value="<?php echo htmlspecialchars($metadata['period'] ?? ''); ?>" required>
                ...
                <div class="word-interface" id="wordInterface">
                    <div class="ribbon">
                        <div class="ribbon-tabs">
                            ...
                            <div class="ribbon-tab" data-tab="rapport">Rapport</div> </div>
                        ...
                        <div class="ribbon-content" id="rapport-content">
                            <div class="ribbon-group">
                                <div class="ribbon-group-title">Actions du rapport</div>
                                <div class="ribbon-buttons">
                                    <div class="ribbon-button" onclick="saveReport()">
                                        <i class="fas fa-save"></i>
                                        <span>Enregistrer</span>
                                    </div>
                                    <div class="ribbon-button" onclick="deleteReport()">
                                        <i class="fas fa-trash-alt"></i>
                                        <span>Supprimer</span>
                                    </div>
                                    <div class="ribbon-button" onclick="window.location.href='mes_rapports.php'">
                    ...
                    <div class="document-container" id="document-container">
                        <div class="cover-page" id="cover-page">
                            <div class="cover-university" id="university-display"><?php echo htmlspecialchars($metadata['university'] ?? 'Université XYZ'); ?></div>
                            <div class="cover-title" id="report-title-display">RAPPORT DE STAGE</div>
                            <div class="cover-academic-year" id="academic-year-display" style="font-size: 12pt; margin-bottom: 1cm;"><?php echo htmlspecialchars($metadata['academicYear'] ?? ''); ?></div>
                            <div class="cover-meta">
                                <div class="meta-top">
                                    <div class="meta-left" id="period-display">Période: <?php echo htmlspecialchars($metadata['period'] ?? 'JJ/MM/AAAA - JJ/MM/AAAA'); ?></div>
                                    <div class="meta-right" id="company-display">Entreprise: <?php echo htmlspecialchars($metadata['companyName'] ?? 'Nom Entreprise'); ?></div>
                                </div>
                                <div class="meta-bottom">
                                    <div class="meta-left" id="student-display">Étudiant: <?php echo htmlspecialchars($metadata['studentName'] ?? 'Prénom NOM'); ?> (<?php echo htmlspecialchars($metadata['studentId'] ?? ''); ?>)</div>
                                    <div class="meta-right" id="supervisor-display">Encadrant: <?php echo htmlspecialchars($metadata['supervisorName'] ?? 'Prénom NOM'); ?></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="page" id="page-1">
                            ...
                            <div class="page-content" contenteditable="true">
                                <?php echo $reportContent; // Load existing content or default ?>
                            </div>
                            <div class="footer">
    ...
    <script>
        ...
        let currentReportId = <?php echo json_encode($_SESSION['current_report_id'] ?? null); ?>;

        // Message Modal elements (duplicated for convenience, but ideally centralized)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (showEditor) {
                ...
                updatePageNumbers();
                // Load report content into the editor only if coming from an existing report
                if (currentReportId) {
                    const firstPageContentDiv = document.querySelector('#page-1 .page-content');
                    if (firstPageContentDiv) {
                        firstPageContentDiv.innerHTML = <?php echo json_encode($reportContent); ?>;
                    }
                }
            } else {
            ...
            document.getElementById('reportMetadataForm').addEventListener('submit', function(e) {
                ...
                if (!/^\d{2}\/\d{2}\/\d{2} - \d{2}\/\d{2}\/\d{2}$/.test(periodValue)) {
                    periodError.textContent = "Le format doit être JJ/MM/AA - JJ/MM/AA.";
                    e.preventDefault();
                    return;
                } else {
                    periodError.textContent = "";
                    const [startDateStr, endDateStr] = periodValue.split(' - ');
                    const parseDate = (s) => {
                        const [day, month, year] = s.split('/').map(Number);
                        // Convert 'YY' to 'YYYY' based on a common pivot year (e.g., 2000)
                    ...
                    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
                        periodError.textContent = "Dates invalides.";
                        e.preventDefault();
                        return;
                    }
                    if (startDate >= endDate) {
                        periodError.textContent = "La date de début doit être antérieure à la date de fin.";
                        e.preventDefault();
                        return;
                    }
                    ...
                    if (diffDays < 30) { // Approximately one month
                        periodError.textContent = "La période de stage doit durer au moins un mois.";
                        e.preventDefault();
                        return;
                    }
                }
                ...
                metadata = {
                    ...
                    companyName: document.getElementById('companyName').value === 'other' ? document.getElementById('newCompanyName').value : document.getElementById('companyName').value,
                    period: document.getElementById('period').value,
                    academicYear: document.getElementById('academicYear').value
                };
                
                // Update cover page immediately on client-side
                updateCoverPage();
            });
            ...
            document.getElementById('saveDraftBtnForm').addEventListener('click', saveReport);
            ...
            // Toggle new company input based on selection
        ...
        function toggleNewCompanyInput() {
            ...
            if (companySelect.value === 'other') {
                ...
                newCompanyNameInput.value = ''; // Clear input if not "other"
            }
        }
        ...
        function updateCoverPage() {
            document.getElementById('report-title-display').textContent = 'RAPPORT DE STAGE'; // Fixed text
            document.getElementById('university-display').textContent = metadata.university;
            document.getElementById('period-display').textContent = `Période: ${metadata.period}`;
            document.getElementById('company-display').textContent = `Entreprise: ${metadata.companyName}`;
            document.getElementById('student-display').textContent = `Étudiant: ${metadata.studentName} (${metadata.studentId})`;
            document.getElementById('supervisor-display').textContent = `Encadrant: ${metadata.supervisorName}`;
            document.getElementById('academic-year-display').textContent = metadata.academicYear;
        }
        ...
        function spellCheck() {
            ...
            const commonWords = new Set(["le", "la", "les", "un", "une", "des", "je", "tu", "il", "elle", "nous", "vous", "ils", "elles", "est", "sont", "ai", "as", "a", "avons", "avez", "ont", "être", "avoir", "dans", "sur", "avec", "pour", "par", "et", "ou", "mais", "où", "qui", "que", "ce", "cette", "ces", "mon", "ma", "mes", "ton", "ta", "tes", "son", "sa", "ses", "rapport", "stage", "entreprise", "nous", "avons", "été", "situé", "dans", "le", "cadre", "d'un", "projet", "developpement", "application"]); // Very limited dictionary
        ...
        // --- Report Save/Delete Functions ---
        async function saveReport() {
            showLoading(true);
            const reportContent = document.getElementById('page-1').parentNode.innerHTML; // Get all pages' HTML
            ...
            try {
                const response = await fetch(window.location.href, {
                    ...
                    body: new URLSearchParams({
                        action: 'save_report',
                        report_id: currentReportId, // Will be null for first save, then populated
                ...
                if (result.success) {
                    ...
                    if (result.report_id) {
                        currentReportId = result.report_id; // Update global ID for subsequent saves
                    }
                } else {
                ...
                showAlert('Erreur lors de l\'enregistrement du rapport. Veuillez réessayer.', 'error');
            } finally {
                showLoading(false);
            }
        }
        ...
        async function deleteReport() {
            if (!currentReportId) {
                showAlert('Aucun rapport à supprimer.', 'warning');
                return;
            }
            ...
            try {
                ...
                if (result.success) {
                    showAlert(result.message, 'success');
                    currentReportId = null; // Clear ID
                    // Optionally, reset the form and editor to initial state
                    document.getElementById('metadataForm').reset();
                    document.getElementById('metadataForm').style.display = 'block';
                    document.getElementById('wordInterface').style.display = 'none';
                    // Clear editor content visually
                    document.querySelector('#page-1 .page-content').innerHTML = '<p><br></p>';
                ...
                showAlert('Erreur lors de la suppression du rapport. Veuillez réessayer.', 'error');
            } finally {
                showLoading(false);
            }
        }
        ...
        function finalSubmit() {
            if (!currentReportId) {
                showAlert("Veuillez d'abord enregistrer le rapport.", "warning");
                return;
            }

            if (confirm("Êtes-vous sûr de vouloir soumettre définitivement ce rapport ? Une fois soumis, vous ne pourrez plus le modifier.")) {
                // In a real application, you would send the final content and metadata to the server via AJAX
                // and update the report status to 'soumis'.
                showAlert("Rapport soumis avec succès! Le statut a été mis à jour.", 'success');
                // You would typically make an AJAX call here to update the 'statut' field in your 'rapports' table
                // e.g., action: 'update_report_status', report_id: currentReportId, status: 'soumis'
            }
        }
