...
// Récupération des informations utilisateur connecté (Assumé pour la Commission)
$loggedInUserId = $_SESSION['user_id'] ?? null;
$loggedInUserType = $_SESSION['user_type'] ?? 'enseignant'; // Assumé si utilisateur est membre de la commission
$currentUserName = 'Membre Commission'; // Placeholder
$currentUserEmail = 'commission@sygecos.edu'; // Placeholder

if ($loggedInUserId) {
    try {
        // Tenter de récupérer le nom et l'email de l'enseignant pour l'utilisateur connecté
        ...
        error_log("Erreur récupération info enseignant connecté: " . $e->getMessage());
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        switch ($action) {
            case 'charger_messages':
                ...
                $sqlBase = "
                    ...
                    -- Removed JOIN for entreprise as sender
                    ...
                    -- Removed JOIN for entreprise as receiver
                ...
                // Pour les dossiers 'brouillons' et 'corbeille', nous avons besoin de colonnes dédiées dans la DB.
                // Sans elles, nous les laissons vides pour l'instant ou ajoutons des conditions si ces colonnes existent.
                ...
                // Pour les comptes (reception, non_lus, envoyes, brouillons, corbeille)
                $sqlCounts = "SELECT
                ...
                // Brouillons et corbeille restent à 0
            ...
            case 'envoyer_message':
                ...
                $messageType = $_POST['message_type'] ?? 'general'; // Default 'general' for commission
            ...
            case 'marquer_lu':
                ...
                // Assurez-vous que l'utilisateur est le RECEIVER des messages pour les marquer lus/non lus
            ...
            case 'supprimer_messages':
                ...
                $placeholders = implode(',', array_fill(0, count($messageIds), '?'));
                // Suppression physique pour cet exemple. En production, vous pourriez déplacer vers une corbeille (is_deleted = 1)
            ...
            case 'get_all_recipients':
                // Récupérer uniquement les enseignants, le personnel administratif et les étudiants
                $query = '%' . ($_POST['query'] ?? '') . '%';
                $recipients = [];
                ...
                // Filtrer l'utilisateur connecté lui-même de la liste des destinataires
                $recipients = array_filter($recipients, function($r) use ($loggedInUserId) {
                    return $r['id_util'] != $loggedInUserId;
                });

                echo json_encode(['success' => true, 'data' => array_values($recipients)]); // array_values pour réindexer
                break;
        ...
        error_log("Erreur dans messagerie_commission.php (AJAX): " . $e->getMessage());
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
    }
    exit;
}

// Si la requête n'est PAS une requête AJAX POST, nous servons la page HTML normale.
...
<head>
    ...
    <title>SYGECOS - Messagerie Commission</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .message-type-personnel, .message-type-officiel { display: none; /* Hide generic types not used by comm for now */ }
        ...
        #reportSpecificFields {
            display: none; /* Hidden by default for Commission */
        }
...
<body>
    <div class="admin-layout">
        <?php include 'sidebar_commision.php'; /* Assurez-vous que ce fichier existe et est correct */ ?>
    ...
    <script>
        ...
        function displayMessagesInList(messages) {
            ...
            messages.forEach(message => {
                ...
                // Check for related_report_id to determine attachment icon (though no actual file for general messages)
                const hasAttachmentIcon = message.related_report_id !== null && message.related_report_id !== undefined; // If 0, it's falsey
        ...
        function displayFullMessage(message) {
            ...
            // Hide download PDF button for general messages
            // This button might be from the other messaging interface, keep it hidden
        ...
        mainComposeForm.addEventListener('submit', async function(e) {
            ...
            try {
                const dataToSend = {
                    ...
                    sujet: subject,
                    contenu: content,
                    message_type: 'general' // Default for commission
                };
        ...
        function updateSidebarCounts(counts) {
            ...
            document.getElementById('countBrouillons').textContent = counts.brouillons; // Still 0
            document.getElementById('countCorbeille').textContent = counts.corbeille;    // Still 0
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            chargerMessages('reception', 1);
            setTimeout(() => {
                showAlert('Messagerie chargée - Membre de la Commission', 'success');
            }, 1000);
        });
